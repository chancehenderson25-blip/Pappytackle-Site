...
interface Item { id: string; anchorId?: string; title: string; vehicle: string; summary: string; category: Exclude<Cat,'all'>; photoSrc: string; photoFullSrc: string; photoAlt: string; }
...
export default function BuildGallery({ items }: { items: Item[] }) {
  ...
  return (
    <div>
        ...
        {filtered.map(i => (
          <button key={i.id} id={i.anchorId} onClick={() => setOpen(i)} className="group block text-left scroll-mt-24">
            <div className="aspect-[4/3] overflow-hidden rounded-[var(--radius-card)] bg-[var(--color-ink-2)]">
              <img src={i.photoSrc} alt={i.photoAlt} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" decoding="async" />
            </div>
      ...
      <Lightbox open={!!open} src={open?.photoFullSrc ?? ''} alt={open?.photoAlt ?? ''} title={open?.title ?? ''} caption={`${open?.vehicle ?? ''} — ${open?.summary ?? ''}`} onClose={() => setOpen(null)} />
    </div>
  );
}
